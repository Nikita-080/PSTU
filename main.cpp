...
bool Split(string s,int row_num)
{
    ...
    case 1:
        if (s==" ")
        {
            cout << row_num << " Ошибка: слова не найдены\n";
            return false;
        }
        else words=1;
        break;
    ...
    if (max_dig==0) cout << row_num << " Ошибка: цифры не найдены\n";
...
int main()
{
    ...
    cout << "номер строки|номер слова|слово|кол-во цифр\n";
    int row_num=0;
    while (!F1.eof())
    {
        string row;
        ...
        else cout<< row_num << " Ошибка: пустая строка\n";
        row_num++;
    }
	F1.close();
	F2.close();
