...
int main()
{
    ...
    {
        cout << "Äàííûå ïîâðåæäåíû èëè íå ñîîòâåòñòâóþò ôîðìàòó\n";
        system("pause");
        return 0;
    }
	...
	for (int i=0;i<4;i++)
    {
        ...
        if (NAME(Data_base[i].fio)==find_name)
        {
            ...
            string new_adress;
            string new_rating;
            cout << "Îáíàðóæåíà èñêîìàÿ ôàìèëèÿ\n";
            cout << "Ââåäèòå ÔÈÎ íîâîãî ýëåìåíòà\n";
            getline(cin,new_fio);
            cout << "Ââåäèòå äàòó ðîæäåíèÿ íîâîãî ýëåìåíòà\n";
            getline(cin,new_birthday);
            cout << "Ââåäèòå àäðåñ íîâîãî ýëåìåíòà\n";
            getline(cin,new_adress);
            cout << "Ââåäèòå ðåéòèíã íîâîãî ýëåìåíòà\n";
